...
function Component:extend(name)
	...
	function class.new(props, context)
		local self = {}
...
function Component:_forceUpdate(newProps, newState)
	...
	if self._reified ~= nil then
		-- We returned an element before, update it.
		self._reified = Reconciler._reconcile(self._reified, newChildElement)
	elseif newChildElement then
		-- We returned nil last time, but not now, so construct a new tree.
		self._reified = Reconciler._reifyInternal(newChildElement, self._parent, self._key, self._context)
	end
